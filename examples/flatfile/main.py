...
def main(schema: Annotated[
    ...
    match output_format:
        ...
        case OutputFormat.excel:
            ...
            custom_header = [v.get("title") or k for k, v in faker.root_schema["properties"].items()]
            with pd.ExcelWriter(output) as excel_writer:
                pd.DataFrame.from_records(fake_data).to_excel(excel_writer, sheet_name='Fake Data', index=False, header=custom_header)
                pd.DataFrame.from_records(more_fake_data).to_excel(excel_writer, sheet_name='More Fake Data', index=False, header=custom_header)
